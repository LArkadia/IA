...
private:
    ...
    public:
        A_star_cell(Cord parent,Cord position, Cord target,uint acummulated,uint (*heuristic)(Cord target,Cord current));     
        void Set_parent(Cord new_parent);   
        void Update(uint new_accumulated,uint new_weigth,Cord new_parent);
        uint Get_accumulated();
        uint Get_heuristic();
        uint Get_weight();
        Cord Get_parent();
        uint8_t Get_texture();
    ...
    Vector<Cord> open;
    Vector<Cord> closed;
    uint (*heuristic_function)(Cord target,Cord current); 
    Cord start,target;
...
public:
    ...
    Pathfinder(String map_file_path,uint (*heuristic)(Cord target,Cord current));
